...
這套流程用來自動抓取中西遊戲新聞候選，交給 Codex 晚上人工審稿後，輸出繁中摘要與貓娘點評，並可透過 Discord webhook 轉發。
...
`.github/workflows/game-news-radar.yml` 只保留手動 `workflow_dispatch`，不再每天自動發遊戲新聞，避免未審稿內容亂發或被 GitHub 排程延遲到錯誤時間。
...
目前是 Codex 審稿模式：程式先依來源、關鍵字、時效和分類產生候選清單；每天晚上由 Codex 讀候選、剔除低價值列表文與弱來源、補上繁中編輯短評，再發 Discord。Bilibili 影片仍維持無腦轉發，不進 AI 審核。

後續遊戲新聞日報的更新、補檢、發送狀態與修正都繼續在同一個 Codex thread 處理，除非主人明確要求開新串。

成功發送狀態以 `data/game_news_sent.json` 為準。若 23:00 主流程失敗或未執行，23:20 補檢會看這個檔案；沒有當天紀錄才重跑。若補跑時間落在台北 00:00-02:59，應用 `--sent-date` 標記前一天的日報。
